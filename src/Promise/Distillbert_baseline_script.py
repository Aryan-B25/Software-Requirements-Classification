...
# Prepare the DistilBERT fine-tuning script path
distilbert_script_path = Path("/mnt/data/distilbert_nfr_classifier.py")
...
OUTPUT_PATH = "./distilbert_outputs"
...
model = DistilBertForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=len(LABELS)).to(device)
...
with open(os.path.join(OUTPUT_PATH, "distilbert_classification_report.txt"), "w") as f:
    f.write(f"Accuracy: {acc:.4f}\\n\\n")
    f.write(report)

with open(os.path.join(OUTPUT_PATH, "distilbert_metrics_summary.json"), "w") as f:
...
plt.ylabel("True")
plt.savefig(os.path.join(OUTPUT_PATH, "distilbert_confusion_matrix.png"))
plt.close()
