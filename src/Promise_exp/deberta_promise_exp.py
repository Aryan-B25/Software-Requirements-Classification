...
from transformers import DebertaTokenizerFast, DebertaForSequenceClassification, AdamW, get_scheduler
...
model = DebertaForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=len(LABELS)).to(device)
optimizer = AdamW(model.parameters(), lr=LR)
lr_scheduler = get_scheduler("linear", optimizer=optimizer, num_warmup_steps=0, num_training_steps=EPOCHS*len(train_dl))
...
}).to_csv(os.path.join(OUTPUT_DIR, "deberta_predictions.csv"), index=False)

with open(os.path.join(OUTPUT_DIR, "deberta_classification_report.txt"), "w") as f:
    f.write(f"Accuracy: {acc:.4f}\n\n")
    f.write(report)

with open(os.path.join(OUTPUT_DIR, "deberta_metrics_summary.json"), "w") as f:
...
plt.ylabel("True")
plt.savefig(os.path.join(OUTPUT_DIR, "deberta_confusion_matrix.png"))
plt.close()

print(f"DeBERTa training complete. Accuracy: {acc:.4f}")
...
soft_output_df.to_csv(os.path.join(OUTPUT_DIR, "deberta_soft_outputs.csv"), index=False)
