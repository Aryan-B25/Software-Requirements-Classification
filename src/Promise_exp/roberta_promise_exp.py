...
from transformers import RobertaTokenizerFast, RobertaForSequenceClassification, AdamW, get_scheduler
...
}).to_csv(os.path.join(OUTPUT_DIR, "roberta_predictions.csv"), index=False)
...
soft_output_df.to_csv(os.path.join(OUTPUT_DIR, "roberta_soft_outputs.csv"), index=False)
