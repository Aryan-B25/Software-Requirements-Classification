...
}).to_csv(os.path.join(OUTPUT_DIR, "norbert_predictions.csv"), index=False)

with open(os.path.join(OUTPUT_DIR, "norbert_classification_report.txt"), "w") as f:
    f.write(f"Accuracy: {acc:.4f}\n\n")
    f.write(report)

with open(os.path.join(OUTPUT_DIR, "norbert_metrics_summary.json"), "w") as f:
...
plt.ylabel("True")
plt.savefig(os.path.join(OUTPUT_DIR, "norbert_confusion_matrix.png"))
plt.close()

print(f"NoRBERT training complete. Accuracy: {acc:.4f}")
