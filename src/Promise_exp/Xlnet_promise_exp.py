...
model = XLNetForSequenceClassification.from_pretrained(MODEL_NAME, num_labels=len(LABELS)).to(device)
...
}).to_csv(os.path.join(OUTPUT_DIR, "xlnet_predictions.csv"), index=False)

with open(os.path.join(OUTPUT_DIR, "xlnet_classification_report.txt"), "w") as f:
    f.write(f"Accuracy: {acc:.4f}\n\n")
    f.write(report)

with open(os.path.join(OUTPUT_DIR, "xlnet_metrics_summary.json"), "w") as f:
...
plt.ylabel("True")
plt.savefig(os.path.join(OUTPUT_DIR, "xlnet_confusion_matrix.png"))
plt.close()

print(f"XLNet training complete. Accuracy: {acc:.4f}")
...
soft_output_df.to_csv(os.path.join(OUTPUT_DIR, "xlnet_soft_outputs.csv"), index=False)
